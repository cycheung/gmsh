...
class simpleFunctionTime : public simpleFunction<scalar>{
  protected :
    ...
    bool timeDependency;
  public :
    simpleFunctionTime(scalar val=0, bool td=true, double t=1.) : simpleFunction<scalar>(val), _val(val), time(t), timeDependency(td){}; // time=1 by default to avoid set time for Static linear Scheme
    ~simpleFunctionTime(){};
