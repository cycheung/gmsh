...
OL.parameter DISTANT.number(1,,Flag remote host); DISTANT.AddChoices(0,1);

% description of the remote host called if case DISTANT=1

OL.iftrue(DISTANT)
  OL.parameter HOST.string(onelab@haddock.mema.ucl.ac.be); 
...
% This allows solving on the local machine but in another directory.
  ...
  OL.parameter WORKDIR.string(tmp/ONELAB/CRYO);
...
%4)Client Postpro pour charger la solution calcul avec Elmer 
